...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        ...
        if (size == backingArray.length) {
            ...
            for (int i = 0; i < backingArray.length; i++) {
                T num = backingArray[(front + i) % backingArray.length];
                tempArr[i + 1] = num;
            }
    ...
    public void addLast(T data) {
        if (data == null) {
            throw new IllegalArgumentException("Null data can not be added to the deque");
        }
        if (size == backingArray.length) {
            T[] tempArr = (T[]) new Object[(size * 2)];
            for (int i = 0; i < backingArray.length; i++) {
            ...
            tempArr[backingArray.length] = data;
            backingArray = tempArr;
            front = 0;
        } else {
            backingArray[mod((front + size % backingArray.length), backingArray.length)] = data;
        }
        size++;
    }
    ...
    public T removeFirst() {
        if (size == 0) {
            throw new NoSuchElementException("The ArrayDeque is empty.");
        } else {
            T first = backingArray[front % backingArray.length];
            backingArray[front % backingArray.length] = null;
            size--;
            front++;
            return first;
        }
    ...
    public T removeLast() {
        if (size == 0) {
            throw new NoSuchElementException("There is nothing to remove since the deque is empty");
        } else {
            int index = mod(((front + size) % backingArray.length) - 1, backingArray.length);
            T last = backingArray[index];
