...
public class LinearProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("Can not put null values into hashmap.");
        }
        ...
        if (table[hash] != null) {
            while (table[hash] != null) {
                if (table[hash].isRemoved()) {
                    table[hash] = new LinearProbingMapEntry<K, V>(key, value);
                    size++;
                    return null;
                } else if (table[hash].getKey().equals(key)) {
                    V hold = table[hash].getValue();
    ...
    public V remove(K key) {
        ...
        for (int i = 0; i < table.length; i++) {
            int ind = Math.abs((hash % table.length) + i) % table.length;
            if (table[ind] == null) {
                throw new NoSuchElementException("");
            } else if (key.equals(table[ind].getKey()) && !(table[ind].isRemoved())) {
                V rem = table[ind].getValue();
                table[ind].setRemoved(true);
                size--;
                return rem;
            }
        }
        throw new NoSuchElementException("The key is not included in the hashmap.");
    }
    ...
    public V get(K key) {
        ...
        for (int i = 0; i < table.length; i++) {
            int ind = Math.abs((hash % table.length) + i) % table.length;
            if (table[ind] == null) {
                throw new NoSuchElementException("The element is not included in the hashmap.");
            } else if (key.equals(table[ind].getKey()) && !(table[ind].isRemoved())) {
                return table[ind].getValue();
            }
        }
        throw new NoSuchElementException("This element is not included in the hashmap.");
    }
    ...
    public boolean containsKey(K key) {
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            if (table[ind] == null) {
                return false;
            } else if (!(table[ind].isRemoved()) && key.equals(table[ind].getKey())) {
                return true;
            }
        }
        return false;
    ...
    public void resizeBackingTable(int length) {
        if (length <= 0 || length < size) {
            throw new IllegalArgumentException("The inputted length is invalid. ");
        }
        ...
        for (int i = 0; i < table.length; i++) {
            if (table[i] != null && !(table[i].isRemoved())) {
                ...
                for (int j = 0; j < length; j++) {
                    int ind = Math.abs((hash % length) + j) % length;
                    if (temp[ind] == null) {
                        temp[ind] = table[i];
                        break;
                    }
